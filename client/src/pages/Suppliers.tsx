...
export default function Suppliers() {
  ...
  const { data: items, isLoading } = trpc.supplier.list.useQuery({ tenantId: 1 });

  const createMutation = trpc.supplier.create.useMutation({
    onSuccess: () => {
      toast.success("Supplier criado(a) com sucesso!");
  ...
  const updateMutation = trpc.supplier.update.useMutation({
    onSuccess: () => {
      toast.success("Supplier atualizado(a) com sucesso!");
      utils.supplier.list.invalidate();
      handleClose();
    },
    onError: (error) => toast.error(error.message),
  ...
  const deleteMutation = trpc.supplier.delete.useMutation({
    onSuccess: () => {
      toast.success("Supplier deletado(a) com sucesso!");
      utils.supplier.list.invalidate();
    },
    onError: (error) => toast.error(error.message),
  });
  ...
  const handleEdit = (item: any) => {
    ...
    setFormData({
      name: item.name || "", cnpj: item.cnpj || "", email: item.email || "", phone: item.phone || ""
    });
    setOpen(true);
  };
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-400 mt-1">Gerencie os suppliers do sistema</p>
          </div>
        ...
        <Card className="bg-slate-800 border-slate-700">
          <CardHeader>
            <CardTitle className="text-white">Lista de Suppliers</CardTitle>
                ...
                <TableBody>
                  {items?.map((item) => (
                    <TableRow key={item.id} className="border-slate-700">
                      ...
                      <TableCell className="text-slate-400">{item.cnpj || "-"}</TableCell>
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogContent className="bg-slate-800 border-slate-700 text-white">
            <DialogHeader>
              <DialogTitle>{editingId ? "Editar" : "Novo(a)"} Supplier</DialogTitle>
